...
app.put('/update-photo/:id', async (req, res) => {
  try {
    const { damageClass, comment, surveyStartDate, surveyEndDate } = req.body;
    ...
    if (surveyStartDate && surveyEndDate) {
      ...
      if (startDate > endDate) {
        return res.status(400).json({ 
          message: 'Survey start date cannot be after end date' 
        });
      }
    }
    ...
    const updates = {};
    if (damageClass) updates.damageClass = damageClass;
    if (comment) updates.comment = comment;
    if (surveyStartDate) updates.surveyStartDate = new Date(surveyStartDate);
    if (surveyEndDate) updates.surveyEndDate = new Date(surveyEndDate);
